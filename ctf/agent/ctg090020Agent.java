...
public class ctg090020Agent extends Agent {
	...
	*		ctg090020Agent
	*
	*		Initialize current Agent and add to agents ArrayList
	*		
	**********************************************************************/
    public ctg090020Agent() {
	...
	/**********************************************************************
	*
	*		Agent class containing agent intiialization (for agent memory)
	*		--Agent
	*		
	**********************************************************************/
	...
	*		getMove
	*		
	*		Determines next move of current Agent
	*		
	**********************************************************************/
    public int getMove(AgentEnvironment inEnvironment) {
        ...
        switch (me.currentObjective) {
            case SEEK_ENEMY_BASE:
            	whatToDo = quickestSeek(enemyBase);
	...
	/**********************************************************************
	*
	*		Determine if enemy is obs nearby
	*
	*		(Determine previous obsticals after move and current obsticals 
	*		 before moving and if new obst exist then it is enemy)
	*
	*		If obst is enemy then move away from obst but towards goal
	*		
	**********************************************************************/
	...
	/**********************************************************************
	*
	*		Determine if teammate is nearby
	*
	*
	*		If teammate is obstical move away but towards goal in way such
	*		that they will not get stuck
	*		
	**********************************************************************/		

	/**********************************************************************
	*
	*		Determine if moves are repeated and obst are the same
	*
	*		If moves are repeated 2 or 3 times without change in obst then
	*		move away using some kind of heuristic
	*		
	**********************************************************************/
    ...
    ****************************************************************/
	...
	int mapHeight = 10; //for now since we know map /(can calculate by moving agents to base)
    Place ourBase;
    Place enemyBase;
    ...
    *		--defend
   	...
   	*		--createDefendingWall
   	*		--seek
	*		--getLeastVistitedSurroundingSquares
	*		--getNumTimesVisited
	*		--checkForSurroundingEnemyAgentsWithOurFlag
	*		--goRandomOpenDirection	
	*		--pickRandomlyBetween
	*		--randomInt
	*		--pickRandomlyBetween
	*		--moveToOneOfLeastVisitedSquares
	*		--directionsOkay
	*		--oscillatingMoveCheck
	*		--smallCircleCheck
	*		--getLastPosition
	*		--getXPositionsBack
	*		--getLastMove
	...
	*		--preferSameDirectionAsPreviousMove
	*		--preferOppositeDirectionOfPreviousMove
	*		--plantMine
	*		--moveNorth
	*		--moveEast
	*		--moveSouth
    ...
    ***********************************************************************/
    ...
    public int quickestSeek(Place place) {
        ...
        Coordinate currentPos = me.position;
        Coordinate dest = place.position;
        ArrayList<Coordinate> blockedList = new ArrayList<Coordinate>();
        ...
        for (int i=0; i<mapHeight; i++)
        {
        	...
        	{
        		if (map[i][j] == 'X')
        			blockedList.add(new Coordinate(i,j));

        	}
        }

        for (int j=0; j<blockedList.size(); j++)
        	System.out.println("Coordinates added to blockedList:" + blockedList.get(j));

        ArrayList path = findShortestPath(currentPos, dest, blockedList);

        return whatToDo;

    }

    public ArrayList findShortestPath(Coordinate currentPos, Coordinate dest, ArrayList<Coordinate> blockedList)
    {
    ...
    *		--positionIsNorthWest
    *		--positionIsSouthWest
    *		--blockedInThreeDirections
    *		--getOnlyFreeDirection
    *		--getNameFromAgentAction
    *		--getDirectionName
    *		--getAgentActionFromDirection
    *		--getDirectionFromAgentAction
    *
    *
    ***************************************************************/
